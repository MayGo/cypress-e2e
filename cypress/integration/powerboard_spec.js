...
describe('PowerBoard', function() {
    it('<title> is "RWE Operator Dashboard"', function() {
        ...
        cy.title({ timeout: 30000 }).should('include', 'RWE Operator Dashboard');
    });

    it(`park total chart shows data`, function() {
        cy
            .get('.park-total path.park-total')
    ...
    it('turbine shows active power', function() {
        cy
            .get('.matrix-container .turbine-compact-component')
            .first()
            .within(function() {
    ...
    context('Filter', function() {
        ...
        it('Click on Unavailable filter dimmes all except unavailable', function() {
            ...
            filterBtn.click();
            cy
                .get('.matrix-container .turbine-compact-component.dimmed')
                .should('have.length', TURBINES_SIZE - this.unavailableNr);
        });
    });
});
