...
describe('Datahub', function() {
    ...
    it('clicking search result item opens it in new window', function() {
        ...
        cy
            .get('.datasets-header')
            .should('contain', 'Datasets that match your filter selection: 1');
    ...
    context('Dataset: fmi_silam_global05', function() {
        it('<title> is "Planet OS - Finnish Meteorological Institute - System for Integrated modeLling of Atmospheric coMposition (SILAM)"', function() {
            ...
            cy
                ...
                .should(
                    'equals',
                    'Planet OS - Finnish Meteorological Institute - System for Integrated modeLling of Atmospheric coMposition (SILAM)'
                );
        });

        it('Loads overlay image', function() {
            cy
                .get('.leaflet-overlay-pane img', { timeout: MAP_OVERLAY_TIMEOUT })
                .as('variableNames');

            cy
                .get('@variableNames')
                .should('have.attr', 'src')
                .and('contains', 'data:image/png;base64,');
        });
        it('Publisher is "Finnish Meteorological Institute (FMI)"', function() {
            cy
                ...
                .contains('Publisher')
                .next()
                .should('contain', 'Finnish Meteorological Institute (FMI)');
        });
    ...
    context('Searching', function() {
        ...
        it('by drawing a rectangle on map - searches by map selection', function() {
            ...
            cy
                .get('.datasets-header')
                .should('contain', 'Datasets that match your filter selection');
        });
    });
});
